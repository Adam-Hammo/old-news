"""How far apart two requests to one host are allowed to be.

Pure functions producing job options; Postgres does the serialising, so nothing
here holds timestamps, semaphores or a registry that grows forever.
"""
...
def gap_for(host: str, *, minimum: float, crawl_delays: Mapping[str, float]) -> float:
    ...
    A publisher asking for less traffic may only lengthen the wait, never shorten it.
    """
...
def stagger(
    ...
    The lock alone would let one publisher's feeds run back-to-back as fast as each
    poll finishes; this puts a floor under the gap.
    """
