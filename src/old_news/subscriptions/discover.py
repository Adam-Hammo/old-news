...
from old_news.fetch import Fetcher, FetchError, fetchable
...
def _attributes(tag: bytes) -> dict[str, str]:
    return {
        name.decode(errors="replace").lower(): value.strip(b"\"'").decode(errors="replace").strip()
...
async def discover(url: str, fetcher: Fetcher) -> str | None:
    ...
    candidates = [u for u in feeds_in(response.body, base_url=response.url) if fetchable(u)]
